...
@IBDesignable class TLFloatLabelTextField: UITextField {
    ...
    override func textRect(forBounds bounds:CGRect) -> CGRect {
        ...
        r = r.inset(by: UIEdgeInsets(top: top, left: 0.0, bottom: 8.0, right: 0.0))
        return r.integral
    }
    ...
    fileprivate func setViewPosition() {
        bottomLineView.frame = CGRect(x: 0, y: self.frame.size.height - 1, width:  self.frame.size.width, height: 1)
    }
    ...
    fileprivate func showTitleLabel(_ animated:Bool) {
        ...
        UIView.animate(withDuration: dur, delay:0, options: [UIView.AnimationOptions.beginFromCurrentState, UIView.AnimationOptions.curveEaseOut], animations:{
            ...
            if let controller = GNavigation.shared.NavigationController.topViewController {
                if controller is VCChangePassword {
                    if (self.text!.isEmpty) {
                        if self.placeHolderColor == .clear {
                            self.placeHolderColor = self.color
                        }
                    }
                }else{
